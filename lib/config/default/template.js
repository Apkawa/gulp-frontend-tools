...
var globals = {
  static: function _static(path) {
    var config = require('..');
    var static_root = config.get('project.static_root');
    return _url3.default.resolve(static_root, path.replace(/^\/+/, ''));
  },
...
var opts = {
  ...
  createEnv: function createEnv(template_options, global_config) {
    var root = template_options.root,
        extra = _objectWithoutProperties(template_options, ['root']);

    var env = new _nunjucks2.default.Environment(new _nunjucks2.default.FileSystemLoader(root, { noCache: true }), extra);
