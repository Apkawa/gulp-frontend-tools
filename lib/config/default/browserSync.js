...
function getBSConfig(config) {
  ...
  var _config$browserSync = config.browserSync,
      server = _config$browserSync.server,
      _config$browserSync$r = _config$browserSync.routes,
      routes = _config$browserSync$r === undefined ? {} : _config$browserSync$r,
      _config$browserSync$m = _config$browserSync.middleware,
      middleware = _config$browserSync$m === undefined ? [] : _config$browserSync$m,
      proxyOptions = _config$browserSync.proxy;


  var bsOptions = _lodash2.default.omit(config.browserSync, ['browserSync', 'getBSConfig', 'proxy', 'routes', 'middleware', 'server']);
  ...
  var compiledBsOptions = _extends({
    ...
    server: _extends({
      ...
      middleware: [].concat(_toConsumableArray(buildProxyList(proxyOptions)), _toConsumableArray(getWebpackMiddlewares(webpack_options)), _toConsumableArray(middleware))
    }, server),
    plugins: []
  }, bsOptions);
