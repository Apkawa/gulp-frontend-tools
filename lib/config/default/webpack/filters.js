...
function productionFilter(webpack_options, config) {
  ...
  if (project_webpack.uglify) {
    var opts = project_webpack.uglify;
    if (_lodash2.default.isBoolean(opts)) {
      opts = {
        sourceMap: true,
        uglifyOptions: {
          ecma: 8,
    ...
    plugins.push(new _uglifyjsWebpackPlugin2.default(opts));
  }
  if (project_webpack.babel_minify) {
    var _opts = project_webpack.babel_minify;
    if (_lodash2.default.isBoolean(_opts)) {
      _opts = [{}, {
        comments: false
      }];
    }
    plugins.push(new (Function.prototype.bind.apply(_babiliWebpackPlugin2.default, [null].concat(_toConsumableArray(_opts))))());
  }
  ...
  return _extends({}, webpack_options, {
    ...
    plugins: [].concat(_toConsumableArray(webpack_options.plugins), [new _webpack2.default.LoaderOptionsPlugin({
      minimize: true,
      debug: false
    }), new _webpack2.default.NoEmitOnErrorsPlugin(), new _imageminWebpackPlugin2.default(_extends({
      disable: process.env.NODE_ENV !== 'production' }, config.webpack.imageminPlugin))], plugins)
  });
}
