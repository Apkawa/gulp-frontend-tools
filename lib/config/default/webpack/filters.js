...
function productionFilter(webpack_options, config) {
  ...
  return _extends({}, webpack_options, {
    ...
    plugins: [].concat(_toConsumableArray(webpack_options.plugins), [new _webpack2.default.LoaderOptionsPlugin({
      minimize: true,
      debug: false
    }), new _webpack2.default.NoEmitOnErrorsPlugin(), new _webpack2.default.optimize.UglifyJsPlugin(_extends({
      minimize: true,
      beautify: false,
      comments: false,
      sourceMap: false,
    ...
    }, project_webpack.uglify)), new _imageminWebpackPlugin2.default(_extends({
      disable: process.env.NODE_ENV !== 'production' }, config.webpack.imageminPlugin))])
  });
}
