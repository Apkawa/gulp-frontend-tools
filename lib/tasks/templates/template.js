...
exports.default = function (gulp, config) {
    ...
    var TEMPLATE_DIST_ROOT = _.get(config, 'project.template.dist');
    ...
    gulp.task('templates:jinja2', function () {
        return gulp.src(TEMPLATE_ROOT + '/**/*.{jinja2,html,j2}', { base: TEMPLATE_ROOT }).pipe(ignore.exclude(_ignore_template2.default)).pipe(debug({ title: "template" })).pipe(data(getJsonData)).pipe(plumber(errorHandler)).pipe(nunjucksRender(template_options)).pipe(plumber.stop()).pipe(gulp.dest(TEMPLATE_DIST_ROOT)).pipe(browserSync.stream());
    });
};
