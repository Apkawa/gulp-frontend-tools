...
exports.default = function (gulp, config) {
    ...
    gulp.task('templates:jinja2', function () {
        return gulp.src(TEMPLATE_ROOT + '/**/*.{jinja2,html,j2}', { base: TEMPLATE_ROOT }).pipe(_gulpIgnore2.default.exclude(_ignore_template2.default)).pipe((0, _gulpDebug2.default)({ title: "template" })).pipe((0, _gulpData2.default)(getJsonData)).pipe((0, _gulpPlumber2.default)(errorHandler)).pipe((0, _gulpNunjucksRender2.default)(template_options)).pipe(_gulpPlumber2.default.stop()).pipe(gulp.dest(TEMPLATE_DIST_ROOT)).pipe(browserSync.stream());
    });
};
