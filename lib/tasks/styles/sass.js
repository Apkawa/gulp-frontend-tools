...
exports.default = function (gulp, config) {
    ...
    gulp.task('sass', function () {
        var context = config.project.context;
        var variables = _lodash2.default.fromPairs(_lodash2.default.map(context, function (v, k) {
            return ['$' + k, v];
        }));
        console.log(variables);

        var stream = gulp.src(APP_PATH.scss + '/**/*.scss').pipe((0, _gulpPreprocess2.default)({ context: config.project.context })).pipe((0, _gulpSassVariables2.default)(variables)).pipe(_gulpSourcemaps2.default.init()).pipe((0, _gulpSassGlob2.default)(sassOpts)).pipe((0, _gulpSass2.default)(sassOpts).on('error', _gulpSass2.default.logError));
